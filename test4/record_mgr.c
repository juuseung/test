...
RC getRecord (RM_TableData *rel, RID id, Record *record) {
	...
	else return RC_TUPLE_NOT_FOUND;
...
RC startScan (RM_TableData *rel, RM_ScanHandle *scan, Expr *cond) {
	...
	if (cond == NULL) return RC_INVALID_SCAN_CONDITION;
...
RC next (RM_ScanHandle *scan, Record *record) {
	...
	if (RM_scan->cond == NULL) return RC_INVALID_SCAN_CONDITION;
...
// createRecord creates an empty record and return that ptr back to the user
